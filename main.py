...
async def _auto_triage_queue() -> None:
    ...
    logger.warning(
        f"Queue Surgeon: {queue} unprocessed — triaging up to {triage_limit}..."
    )
    ...
    if discard_ids:
        ...
        logger.info(
            f"Triage discarded {len(discard_ids)} noise msgs ({queue - len(discard_ids)} remain)."
        )
...
async def processing_loop() -> None:
    ...
    async def process_one_batch(msgs: Sequence[dict[str, Any]]) -> None:
            ...
            try:
                async with _AI_SEMAPHORE:
                    try:
                        ...
                        logger.warning(
                            f"AI rate limits exhausted — requeuing {len(msgs)} msgs for later."
                        )
                        ai_failed = True
        ...
        if promos is None:
            logger.warning(
                f"AI returned None — incrementing failure count for {len(msgs)} msgs."
            )
            await db.increment_ai_failure_count(msg_ids)
            shared.record_ai_outcome(success=False)
            await _release_claims()
        ...
        try:
            ...
            if filtered:
                ...
                pending_conf_updates: list[dict] = []
                ...
                for p in filtered:
                    ...
                    if p.status != "expired" and age_sec < 7200:
                        ...
                        if confidence >= 45:
                            ...
                            async with _recent_alerts_lock:
                                _recent_alerts_history.append(
                                    {"brand": brand_key, "summary": p.summary}
                                )
                ...
                # ⚡ Bolt Optimization: Batch process pending_confirmations to avoid N+1 queries
                if pending_conf_updates and db.conn:
                    brands_to_fetch = list(
                        {u["brand_key"] for u in pending_conf_updates}
                    )
                    placeholders = ",".join(["?"] * len(brands_to_fetch))

                    async with db.conn.execute(
                        f"SELECT brand, id, corroboration_texts FROM pending_confirmations WHERE brand IN ({placeholders})",
                        brands_to_fetch,
                    ) as cur:
                        existing_rows = await cur.fetchall()

                    existing_map = {
                        row[0]: {"id": row[1], "texts": row[2]} for row in existing_rows
                    }

                    local_state = {}
                    for b in existing_map:
                        try:
                            local_state[b] = json.loads(existing_map[b]["texts"])
                        except Exception:
                            local_state[b] = []

                    inserts = []
                    updates = {}  # brand -> (texts list, added count)

                    for u in pending_conf_updates:
                        brand_key = u["brand_key"]
                        snippet = u["snippet"]
                        if brand_key in existing_map or brand_key in local_state:
                            if brand_key not in local_state:
                                local_state[brand_key] = []
                            if brand_key not in updates:
                                updates[brand_key] = [local_state[brand_key], 0]

                            if snippet and snippet not in local_state[brand_key]:
                                local_state[brand_key].append(snippet)
                                updates[brand_key][1] += 1
                        else:
                            local_state[brand_key] = [snippet] if snippet else []
                            inserts.append(
                                (
                                    brand_key,
                                    u["p_data_json"],
                                    ...
                                    u["timestamp"],
                                    u["confidence"],
                                    json.dumps(local_state[brand_key]),
                                )
                            )

                    update_data = []
                    for b, (texts_list, added_count) in updates.items():
                        if b in existing_map and added_count > 0:
                            update_data.append(
                                (
                                    added_count,
                    ...
                    if update_data:
                        await db.conn.executemany(
                            "UPDATE pending_confirmations SET corroborations=corroborations+?, corroboration_texts=? WHERE id=?",
                            update_data,
                        )
                    ...
                    if inserts:
                        ...
                        inserts_with_exp = [(*i, expires_at) for i in inserts]
                        await db.conn.executemany(
                ...
                await db.save_promos_batch(
                    promos_to_save, [p[0] for p in promos_to_save]
                )
    ...
    while True:
        try:
            ...
            priority_raw = await db.get_unprocessed_recent(
                minutes=10, batch_size=priority_cap + 50
            )

            combined: list[Any] = []
            async with _in_progress_lock:
                ancient = [r for r in ancient_raw if r["id"] not in _in_progress_ids][
                    :ancient_reserve
                ]
                ...
                if backlog_needed > 0:
                    old_raw = await db.get_unprocessed_batch(
                        batch_size=backlog_needed + 200
                    )
                ...
                if ancient:
                    try:
                        oldest_age = max(
                            (
                                datetime.now(timezone.utc) - _parse_ts(r["timestamp"])
                            ).total_seconds()
                            for r in ancient
                        )
            ...
            if fasttext_noise_ids:
                await db.mark_batch_processed(
                    fasttext_noise_ids, skip_reason="fasttext"
                )
                async with _in_progress_lock:
                    for nid in fasttext_noise_ids:
                        _in_progress_ids.pop(nid, None)
                logger.debug(
                    f"🛡️ Traffic Cop: Filtered {len(fasttext_noise_ids)} messages (reason: fasttext)."
                )
            ...
            logger.info(
                f"🧬 Spawning batch: {len(to_ai)} msgs (tasks={shared._active_ai_tasks}, headroom={headroom_pct:.0%}, queue={queue_size})"
            )
...
async def _active_ai_tasks_reconciler() -> None:
    ...
    if counter > live_tasks:
        ...
        logger.warning(
            f"⚠️ _active_ai_tasks counter drift: counter={counter} live_tasks={live_tasks} — reconciling down by {drift}"
        )
...
async def _in_progress_reaper() -> None:
    ...
    if evicted:
        logger.warning(
            f"⚠️ In-progress reaper freed {len(evicted)} stuck claims. Messages will be retried."
        )
...
async def _listener_health_watchdog() -> None:
    ...
    try:
        ...
        if mtproto_connected:
            return
        backoff = min(
            15.0 * (_listener_reconnect_attempts + 1), _LISTENER_RECONNECT_COOLDOWN_SEC
        )
        now_m = time.monotonic()
        ...
        logger.warning(
            f"🔌 Listener watchdog: socket disconnected — forcing reconnect."
        )
...
async def main() -> None:
    ...
    if BOOT_CATCHUP_WINDOW > 3600:
        logger.info(
            f"Large gap detected: catching up {BOOT_CATCHUP_WINDOW/3600:.1f} hours."
        )
    ...
    # ── Scheduled jobs ─────────────────────────────────────────────────────────
    scheduler.add_job(
        jobs.image_processing_job,
        "interval",
        minutes=5,
        id="images",
        args=[db, gemini, listener],
        jitter=30,
    )
    scheduler.add_job(
        jobs.brewing_digest_job,
        "cron",
        minute=0,
        hour="0,1,5-23",
        id="brewing_digest",
    ...
    scheduler.add_job(
        jobs.hourly_digest_job,
        "cron",
        minute=1,
        hour="0,1,5-23",
        id="digest",
        args=[db, gemini, bot, WIB],
        jitter=60,
    )
    scheduler.add_job(
        jobs.midnight_digest_job,
        "cron",
        hour=5,
        minute=0,
        id="midnight_digest",
    ...
    scheduler.add_job(
        jobs.time_mention_job, "interval", minutes=5, id="time_mentions", args=[db, bot]
    )
    scheduler.add_job(
        jobs.trend_job, "interval", minutes=20, id="trend_job", args=[db, gemini, bot]
    )
    scheduler.add_job(
        jobs.spike_detection_job,
        "interval",
        minutes=5,
        id="spike",
        args=[db, gemini, bot],
    ...
    scheduler.add_job(
        jobs.dead_promo_reaper_job, "interval", minutes=20, id="reaper", args=[db, bot]
    )
    scheduler.add_job(
        jobs.confirmation_gate_job, "interval", minutes=1, id="confirm_gate", args=[db]
    )
    scheduler.add_job(
        jobs.db_maintenance_job,
        "cron",
        hour="*/4",
        minute=5,
        id="db_maint",
