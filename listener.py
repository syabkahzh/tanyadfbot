...
QUESTION_AMAN_PATTERN = re.compile(r'\b(aman|work|on)\s+(ga|gak|nggak|ya)\b', re.IGNORECASE)
...
def check_fast_path(text: str) -> bool:
    ...
    if QUESTION_AMAN_PATTERN.search(tl):
        return False
...
class TelethonListener:
    ...
    async def _handle_fast_path_from_event(self, event) -> None:
        ...
        if NEG_PATTERN.search(text_lower):
            return
        if QUESTION_AMAN_PATTERN.search(text_lower):
            return
