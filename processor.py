...
_WORD_BOUNDARY_KEYWORDS = re.compile(
    r"\b(off|on|aman|work|bs|jp|mm|nt|cb|qr)\b", re.IGNORECASE
)
_SOCIAL_FILLER = re.compile(
    r"^((wkwk|haha|hehe|iya|noted|oke|ok|makasih|thanks|thx|mantap|gas|bos|"
...
_PROMO = re.compile(
    ...
    r"neo|tmrw|saqu|seabank|hero)\b",
    re.IGNORECASE,
)
...
_STRONG_KEYWORDS: set[str] = {
    ...
    "tukpo",
    "tukar poin",  # tukar poin (point exchange)
    "murce",
    "murmer",  # murah/cheap slang
    "sopi",
    "sopie",  # Shopee misspelling
    "tsel",
    "mytsel",  # Telkomsel slang
    "cgv",
    "xxi",  # cinema brands
    "svip",  # ShopeeFood VIP
    ...
    "kreator",
    "live kreator",  # Shopee/Tokped live creator events
    "kopken",
    "chatime",
    "gindaco",  # food brands seen in data
    "solaria",
    "rotio",  # restaurant brands
    "spx",
    "shopee xpress",  # logistics brand
    "gopay",
    "spay",
    "shopeepay",
    "ovo",  # payment brands
...
class GeminiProcessor:
    ...
    def _estimate_tokens(text: str | list[Any]) -> int:
        ...
        chars = (
            sum(len(str(p)) for p in text) if isinstance(text, list) else len(str(text))
        )
    ...
    async def _call(
        self, contents: Any, config: dict[str, Any], model_id: str, retries: int = 2
    ) -> Any:
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt == retries:
                    logger.error(
                        f"AI call failed after {retries + 1} attempts: {err[:200]}"
                    )
                    self._slots[slot_acquired].release_last()
                    return None
    ...
    async def process_batch(
        ...
        batch_text = "\n---\n".join(
            f"ID:{m['id']} {m['context']}MSG:{m['text'] or ''}" for m in filtered
        )
        config = {
        ...
        logger.info(
            f"Extracted {len(valid)} promos from batch of {len(filtered)} msgs."
        )
        return valid
    ...
    async def summarize_thread(
        ...
        if parent_photo:
            contents.append(
                genai.types.Part.from_bytes(data=parent_photo, mime_type="image/jpeg")
            )
        ...
        return (
            ...
            if response
            else "Thread ini sedang ramai dibicarakan."
        )
    ...
    async def generate_narrative(
        self, messages: Sequence[dict[str, Any] | Any], db: Any = None
    ) -> list[TrendItem]:
