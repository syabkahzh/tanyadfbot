...
_QUESTION_AMAN_PATTERN = re.compile(r'\b(aman|work|on)\s+(ga|gak|nggak|ya)\b', re.IGNORECASE)
_QUESTION_AMAN_NGGA_PATTERN = re.compile(r'\b(aman|work|on)\s+(ngga)\b', re.IGNORECASE)
...
_JSON_MD_BLOCK_PATTERN = re.compile(r'^```json\s*|\s*```$', flags=re.MULTILINE)
...
class OpenAICompatibleClient(BaseAIClient):
    ...
    async def generate_content(self, model: str, contents: Any, config: dict[str, Any], capabilities: dict[str, Any]) -> Any:
        ...
        if response_schema and text:
            try:
                clean_text = _JSON_MD_BLOCK_PATTERN.sub('', text.strip())
                import json
                try:
