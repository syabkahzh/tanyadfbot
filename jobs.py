...
async def time_reminder_job(db: Database, bot: TelegramBot, WIB: Any) -> None:
    ...
    try:
        ...
        if summaries:
            ...
            ft_results = {
                summaries[i]: classifications[i] for i in range(len(summaries))
            }
        ...
        if fired_ids:
            ...
            await db.conn.commit()
            logger.info(
                f"✅ [Job] time_reminder_job fired/marked {len(fired_ids)} promos"
            )
        else:
...
async def hourly_digest_job(
    db: Database, gemini: GeminiProcessor, bot: TelegramBot, WIB: Any
) -> None:
    ...
    try:
        ...
        for r in rows:
            brand = (
                r["brand"]
                if r["brand"] and r["brand"].lower() not in ("unknown", "sunknown", "")
                else "❓"
            )
        ...
        for brand, promos in sorted(by_brand.items()):
            ...
            for r in promos:
                ...
                body_lines.append(
                    f"  {status_icon} {r['summary']}{fast_tag}{link_part}"
                )
        ...
        try:
            async with asyncio.timeout(60):
                ai_summary = await gemini.answer_question(
                    f"Rangkum promo berikut dalam 2-3 kalimat santai. Jam: {hour_label}.",
                    context,
                )
        except TimeoutError:
...
async def midnight_digest_job(
    db: Database, gemini: GeminiProcessor, bot: TelegramBot
) -> None:
...
async def image_processing_job(
    db: Database, gemini: GeminiProcessor, listener: Any
) -> None:
    ...
    try:
            ...
            try:
                ...
                async with asyncio.timeout(30):
                    ...
                    if not msg_obj or not msg_obj.photo:
                        await db.conn.execute(
                            "UPDATE messages SET image_processed=1 WHERE id=?",
                            (msg_id,),
                        )
                        return msg_id
                ...
                if promo:
                    ...
                    promo.queue_time = (
                        datetime.now(timezone.utc) - shared._parse_ts(ts)
                    ).total_seconds() - ai_duration
        ...
        logger.info(
            f"✅ [Job] Finished image_processing_job (processed {len(processed_ids)} imgs)"
        )
    except Exception as e:
...
async def heartbeat_job(
    ...
    try:
        ...
        sorted_slots = sorted(
            gemini._slots.values(), key=lambda s: s.current_usage(), reverse=True
        )
        ...
        for slot in sorted_slots:
            ...
            if usage > 0 or (slot.daily_limit > 0 and daily > slot.daily_limit * 0.8):
                daily_tag = (
                    f" 📅{daily}/{slot.daily_limit}" if slot.daily_limit > 0 else ""
                )
...
async def hot_thread_job(
    ...
    bot: TelegramBot,
    WIB: Any,
    alerted_hot_threads: dict[int, tuple[int, datetime]],
) -> None:
    ...
    try:
        ...
        for t in threads:
            ...
            if t["reply_count"] >= last_count + 3 and cooldown_ok:
                ...
                msg_wib = (
                    shared._parse_ts(t["timestamp"])
                    .astimezone(WIB)
                    .strftime("%H:%M:%S")
                )

                reply_rows = await db.get_thread_replies(
                    t["tg_msg_id"], t["chat_id"], limit=20
                )
                    ...
                    try:
                        ...
                        async with asyncio.timeout(15):
                            downloaded = await listener.client.download_media(
                                await listener.client.get_messages(
                                    t["chat_id"], ids=t["tg_msg_id"]
                                ),
                                bytes,
                            )
                        if isinstance(downloaded, bytes):
                            ...
                            parent_photo = await asyncio.to_thread(
                                _read_file_bytes, downloaded
                            )
                            await asyncio.to_thread(os.remove, downloaded)
                    except asyncio.TimeoutError:
                        logger.warning(
                            f"hot_thread_job: photo download timeout for {t['tg_msg_id']}"
                        )
                    except Exception as e:
                ...
                age_min = (
                    now_ts - shared._parse_ts(t["timestamp"])
                ).total_seconds() / 60
                text = (
...
async def time_mention_job(db: Database, bot: TelegramBot) -> None:
    ...
    try:
        ...
        for r in rows:
            ...
            if (
                ft_label == "__label__JUNK" and ft_conf > 0.85
            ) or not _is_time_signal_worthy(text):
                noise_count += 1
                continue
        ...
        if all_done:
            ...
            if noise_count > 0:
                logger.info(
                    f"⏰ [Job] Marked {noise_count} noise time-mentions as alerted."
                )
...
async def trend_job(db: Database, gemini: GeminiProcessor, bot: TelegramBot) -> None:
    ...
    try:
        ...
        if (
            current_summary == shared._last_trend_alert
...
async def spike_detection_job(
    ...
    try:
        ...
        one_min_ago = (now_utc - timedelta(minutes=1)).strftime(
            "%Y-%m-%d %H:%M:%S+00:00"
        )
        if not db.conn:
            return
        ...
        five_min_ago = (now_utc - timedelta(minutes=5)).strftime(
            "%Y-%m-%d %H:%M:%S+00:00"
        )
        async with db.conn.execute(
        ...
        if count >= 10 and count >= max(avg_per_min * 1.5, 5):
            ...
            sample_lines = [
                f"• {html.escape((m['text'] or '').strip()[:80])}"
            ...
            try:
                async with asyncio.timeout(30):
                    narrative = await gemini.interpret_keywords(
                        hot_words,
                        3,
                        [cast(str, m["text"]) for m in recent_msgs if m["text"]],
                    )
            except TimeoutError:
...
async def dead_promo_reaper_job(db: Database, bot: TelegramBot) -> None:
    ...
    try:
        ...
        if promo_ids_to_reap:
            logger.info(
                f"💀 Reaper: found {len(promo_ids_to_reap)} promos to mark as expired."
            )
